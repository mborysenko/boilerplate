...
const parseStyles = function (input) {
    const attributes = input.split(';');
    const styles = attributes.reduce(function (object, attribute) {
...
export const convertTagAttributes = function (tag) {
    const attributes = tag.attribs;

    const vdomProperties = {
        attributes: {}
    };

    Object.keys(attributes).forEach(function (attributeName) {
        const lowerCased = attributeName.toLowerCase();
        const propInfo = getPropertyInfo(lowerCased);
        ...
        const valueConverter = propertyValueConversions[propInfo.propertyName];
        if (valueConverter) {
